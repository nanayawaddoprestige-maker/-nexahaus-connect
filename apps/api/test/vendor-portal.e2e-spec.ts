...
describe("Vendor portal isolation (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    const vendorRole = await prisma.role.findUniqueOrThrow({
      where: { key: RoleKey.VENDOR },
    });

    const vendorUser = await prisma.user.create({
      data: {
        email: `v.user.${stamp}@nexahaus.test`,
        fullName: "V Vendor",
        passwordHash: hash,
        status: "ACTIVE",
    ...
    const req = await prisma.maintenanceRequest.create({
      data: {
        ...
        statusHistory: {
          create: [
            { toStatus: "REPORTED" },
            { fromStatus: "REPORTED", toStatus: "ASSIGNED" },
          ],
        },
      },
    });
    ...
    vendorToken = (
      await request(app.getHttpServer())
        ...
        .send({ identifier: vendorUser.email, password: PASSWORD })
        .expect(200)
  ...
  afterAll(async () => {
    ...
    await prisma.domainEvent.deleteMany({
      where: { type: "MAINTENANCE_COMPLETED" },
    });
  ...
  it("cannot open another vendor's work order (404, no leak)", async () => {
    const res = await asVendor(
      "get",
      `/api/v1/vendor/work-orders/${otherWorkOrderId}`,
    );
  ...
  it("starts and completes its own job, driving the maintenance request", async () => {
    await asVendor(
      "post",
      `/api/v1/vendor/work-orders/${myWorkOrderId}/start`,
    ).expect(201);
    let req = await prisma.maintenanceRequest.findUniqueOrThrow({
      where: { id: requestId },
    });
    ...
    const done = await request(app.getHttpServer())
      ...
      .set("authorization", `Bearer ${vendorToken}`)
      .send({
        actualCost: { minor: "45000", currency: "GHS" },
        completionNotes: "Replaced the trap.",
      })
      .expect(201);
    expect(done.body.data.status).toBe("COMPLETED");

    req = await prisma.maintenanceRequest.findUniqueOrThrow({
      where: { id: requestId },
    });
    ...
    const event = await prisma.domainEvent.findFirst({
      where: {
        ...
        payload: { path: ["maintenanceRequestId"], equals: requestId },
      },
    });
  ...
  it("cannot reach owner or admin endpoints", async () => {
    ...
    await asVendor("get", `/api/v1/properties/${propertyId}/financials`).expect(
      (r) => {
