...
describe("Tenant portal isolation (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    const ownerUser = await prisma.user.create({
      data: {
        email: `t.owner.${stamp}@nexahaus.test`,
        fullName: "T Owner",
        passwordHash: hash,
        status: "ACTIVE",
    ...
    const tenantUser = await prisma.user.create({
      data: {
        email: `t.tenant.${stamp}@nexahaus.test`,
        fullName: "T Tenant",
        passwordHash: hash,
        status: "ACTIVE",
        emailVerifiedAt: new Date(),
    ...
    for (let m = 0; m < 3; m += 1) {
      ...
      await prisma.rentCharge.create({
        data: {
          ...
          currency: "GHS",
          paidMinor: m === 0 ? 600_000n : 0n,
          status: m === 0 ? "PAID" : "OVERDUE",
        },
      });
      await prisma.rentCharge.create({
        data: {
    ...
    await prisma.payment.create({
      data: {
        ...
        amountMinor: 600_000n,
        currency: "GHS",
        receivedAt: new Date("2027-01-04"),
        method: "MOBILE_MONEY",
        status: "CONFIRMED",
        idempotencyKey: `seed-t-${stamp}`,
      },
    });
  ...
  it("GET /tenant/rent exposes NO owner-level figures", async () => {
    ...
    expect(
      res.body.data.charges.every(
        (c: { amount: { minor: string } }) => c.amount.minor === "600000",
      ),
    ).toBe(true);
  ...
  it("a tenant cannot reach owner or admin endpoints", async () => {
    ...
    await asTenant(`/api/v1/properties/${propertyId}/financials`).expect(
      (r) => {
        if (![403, 404].includes(r.status))
          throw new Error(`expected 403/404, got ${r.status}`);
      },
  ...
  it("a tenant can report maintenance and see it, but not another unit's history via /tenant", async () => {
    ...
    const list = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(
      (list.body.data as { title: string }[]).some(
        (m) => m.title === "Leaking tap",
      ),
    ).toBe(true);
  });
