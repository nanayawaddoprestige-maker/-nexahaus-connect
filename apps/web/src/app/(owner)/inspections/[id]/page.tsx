...
const RATING_TONE: Record<string, "positive" | "warning" | "critical" | "neutral"> = {
  GOOD: "positive",
...
export default function InspectionDetailPage() {
  ...
  const download = useMutation({
    mutationFn: () =>
      api.get<{ url: string }>(`/documents/${data!.reportDocumentId}/download-url`),
  ...
  if (isError || !data) {
    ...
    return (
      <ErrorState
        title={notFound ? "Inspection not found" : "We couldn't load this inspection."}
        onRetry={notFound ? undefined : () => void refetch()}
      />
    );
  }

  const byArea = data.items.reduce<Record<string, typeof data.items>>((acc, it) => {
    (acc[it.area] ??= []).push(it);
    return acc;
  }, {});
  ...
  return (
    <div>
      <button onClick={() => router.push("/inspections")} className="mb-4 text-sm text-ink-muted hover:text-navy-900">
        ← All inspections
      </button>
      <PageHeader
        title={
      ...
      <div className="mb-6 grid grid-cols-1 gap-3 rounded-xl border border-line bg-surface p-4 text-sm sm:grid-cols-3">
        <div>
          ...
          <p className="mt-1 font-medium text-navy-900">{formatDate(data.scheduledFor)}</p>
        </div>
        <div>
          <p className="nx-label">Completed</p>
          <p className="mt-1 font-medium text-navy-900">{formatDate(data.completedAt)}</p>
        </div>
        <div>
                ...
                {items.map((it) => (
                  <li key={it.id} className="border-l-2 border-line pl-3">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-navy-900">{it.label}</span>
                      <StatusBadge status={it.rating} tone={RATING_TONE[it.rating]} />
                    </div>
                    {it.note ? <p className="mt-1 text-sm text-ink-muted">{it.note}</p> : null}
                    {it.recommendation ? (
