...
export default function MaintenancePage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["maintenance", { openOnly, page }],
    queryFn: () =>
      api.list<MaintenanceRow>("/maintenance", {
        query: { openOnly: openOnly ? "true" : undefined, page, pageSize: 15 },
      }),
  ...
  return (
    <div>
      <PageHeader
        title="Maintenance"
        subtitle="Requests across your properties, from report to completion."
      />
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="When a tenant or your manager reports an issue, it appears here with its full history."
        />
      ) : (
        <div className="space-y-3">
          {rows.map((m) => (
              ...
              <Card className="flex items-center justify-between gap-4 transition-shadow hover:shadow-raised">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <StatusBadge status={m.priority} tone={PRIORITY_TONE[m.priority]} />
                    <span className="truncate font-medium text-navy-900">{m.title}</span>
                  </div>
                  <p className="mt-1 text-xs text-ink-subtle">
                    <span className="font-mono">{m.ref}</span> · {m.property.name}
                    {m.unit ? ` · ${m.unit}` : ""} · {titleCase(m.category)}
                    {m.scheduledFor ? ` · scheduled ${formatDate(m.scheduledFor)}` : ""}
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-4">
                  <span className="text-sm tabular-nums text-ink-muted">
                    {formatMoney(m.actualCost ?? m.approvedCost ?? m.estimatedCost)}
                  </span>
