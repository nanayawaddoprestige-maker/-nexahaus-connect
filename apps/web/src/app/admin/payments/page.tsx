...
export default function AdminPaymentsPage() {
  ...
  const [form, setForm] = useState({ leaseId: "", cedis: "", method: "MOBILE_MONEY", reference: "" });
  ...
  const record = useMutation({
    ...
    onSuccess: () => {
      setError(null);
      setForm({ leaseId: "", cedis: "", method: "MOBILE_MONEY", reference: "" });
  ...
  return (
    <div>
      <PageHeader title="Payments" subtitle={meta?.totalItems != null ? `${meta.totalItems} recorded` : undefined} />

      <Card className="mb-6">
        <CardHeader title="Record a payment" description="For payments received outside the online channel (bank deposit, cash handover, MoMo confirmation)." />
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <input className={inputCls} placeholder="Lease ID (UUID)" value={form.leaseId} onChange={(e) => setForm((f) => ({ ...f, leaseId: e.target.value }))} />
          <input className={inputCls} placeholder="Amount (GHS)" inputMode="decimal" value={form.cedis} onChange={(e) => setForm((f) => ({ ...f, cedis: e.target.value }))} />
          <select className={inputCls} value={form.method} onChange={(e) => setForm((f) => ({ ...f, method: e.target.value }))}>
            {["MOBILE_MONEY", "BANK_TRANSFER", "BANK_DEPOSIT", "CASH", "OTHER"].map((m) => (
              <option key={m} value={m}>{titleCase(m)}</option>
            ))}
          </select>
          <input className={inputCls} placeholder="Reference (optional)" value={form.reference} onChange={(e) => setForm((f) => ({ ...f, reference: e.target.value }))} />
          <Button loading={record.isPending} disabled={!form.leaseId.trim() || !Number(form.cedis)} onClick={() => record.mutate()}>
            Record
          </Button>
        </div>
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        {selected.size > 0 ? (
          <Button size="sm" loading={reconcile.isPending} onClick={() => reconcile.mutate()}>
      ...
      {isLoading ? (
        ...
        <EmptyState title="No payments" description="Recorded and online payments appear here." />
      ) : (
            ...
            <tbody>
              {rows.map((p) => (
                <tr key={p.id} className="border-b border-line last:border-0">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-navy-700">{p.ref}</td>
                  ...
                  <td className="px-4 py-3 text-ink-muted">{formatDate(p.receivedAt)}</td>
                  <td className="px-4 py-3">
                    <StatusBadge
                      status={p.reconciliationStatus}
                      tone={p.reconciliationStatus === "RECONCILED" ? "positive" : "neutral"}
                    />
                  </td>
                </tr>
              ))}
