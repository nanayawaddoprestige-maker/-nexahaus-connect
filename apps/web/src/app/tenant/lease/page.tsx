...
export default function TenantLeasePage() {
  const lease = useQuery({ queryKey: ["tenant", "lease"], queryFn: () => api.get<TenantLease>("/tenant/lease") });
  ...
  if (!lease.data) return <EmptyState title="No active lease" description="Contact your NexaHaus property manager." />;
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          l.documentId ? (
            <Button size="sm" variant="secondary" loading={download.isPending} onClick={() => download.mutate(l.documentId!)}>
              Download lease (PDF)
            </Button>
          ) : undefined
        }
      ...
      <Card>
        ...
        <dl className="grid grid-cols-1 gap-x-8 gap-y-3 text-sm sm:grid-cols-2">
          ...
          <Row label="Unit" value={`${l.unit.label}${l.unit.bedrooms ? ` · ${l.unit.bedrooms} bed` : ""}${l.unit.bathrooms ? ` · ${l.unit.bathrooms} bath` : ""}`} />
          <Row label="Term" value={`${formatDate(l.startDate)} – ${formatDate(l.endDate)}`} />
          <Row label="Rent" value={`${formatMoney(l.rent)} / ${l.frequency.toLowerCase()}`} />
          <Row label="Deposit" value={l.deposit ? formatMoney(l.deposit) : "—"} />
            ...
            <ul className="space-y-1 text-sm">
              {l.coTenants.map((t) => (
                <li key={t.name} className="flex justify-between">
                  ...
                  {t.isPrimary ? <span className="text-xs text-ink-subtle">Primary</span> : null}
                </li>
              ))}
            </ul>
          </div>
