  ...
  return (
    <Section id="services">
              ...
              <ul className="mt-4 grid grid-cols-1 gap-x-4 gap-y-1.5 text-sm text-ink-muted sm:grid-cols-2">
                {service.includes.map((item) => (
                  <li key={item} className="flex items-start gap-1.5">
                    <span aria-hidden className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gold-500" />
                    {item}
                  </li>
                ))}
              </ul>
      ...
      <p className="mt-8 max-w-prose text-xs text-ink-subtle">
        Leasing and other regulated real-estate agency activities are provided only where
        NexaHaus holds the required Ghanaian licensing and qualified personnel. NexaHaus does
        not provide legal, valuation, tax or investment advice.
      </p>
    </Section>
  );
}
