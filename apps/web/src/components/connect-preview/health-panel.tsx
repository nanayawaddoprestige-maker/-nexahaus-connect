...
export function HealthPanel() {
  ...
  return (
    <ConnectFrame title="NexaHaus Connect — Property Health" sidebar={<ModuleRail active="Property Health" />}>
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-base font-semibold text-navy-900">Property Health</h3>
          <p className="text-xs text-ink-subtle">Cantonments Court, Block B · updated 3 Sep 2026</p>
        </div>
        <div className="text-right">
          <div className="text-2xl font-semibold tabular-nums text-navy-900">{overall}</div>
        ...
        {CATEGORIES.map((c) => (
          <li key={c.name}>
            ...
            <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-navy-50">
              <div className={cn("h-full rounded-full", band(c.score))} style={{ width: `${c.score}%` }} />
            </div>
          </li>
        ))}
      </ul>
      ...
      <p className="mt-4 text-[11px] text-ink-subtle">
        A NexaHaus management assessment framework — not a professional valuation, legal
